...
void motion_plan::straight(float len_target,float acc,float max_sp,float end_sp)
{
	...
	move_task->rT.is_wallControl_Enable = Non_controll;
	move_task->straight_gain_set.set_sp_gain(16.0, 0.3, 0.0);
	move_task->straight_gain_set.set_om_gain(0.6, 0.1, 0.00);
...
void motion_plan::pivot_turn(float rad_target,float rad_acc,float rad_velo)
{
	...
	move_task->target.radian = 0.0;
	move_task->rT.is_wallControl_Enable = Non_controll;
	move_task->straight_gain_set.set_sp_gain( 16.0, 0.3, 0.00);
	move_task->straight_gain_set.set_om_gain(0.8, 0.0005, 0.00);
	move_task->turn_gain_set.set_sp_gain(16.0, 0.3, 0.00);
	move_task->turn_gain_set.set_om_gain(0.8, 0.0005, 0.00);
	move_task->mouse.x_point = 0.0;
	SensingTask::getInstance().Division_Wall_Correction_Reset();
...
void motion_plan::searchSlalom(const t_param *turn_param)
{
	...
	move_task->straight_gain_set.set_sp_gain(16.0,  0.3, 0.0001);
	move_task->straight_gain_set.set_om_gain(0.6, 0.01, 0.00);
	move_task->turn_gain_set.set_sp_gain(16.0, 0.1, 0.0);
